#######################################################
# MakeFile for AVR microtrollers Arduino based board
# Copyright (c) 2017 Vagner F. Panarello (vpanarello@gmail.com)
...
# read low, high and extended fuses
AVRDUDE_FUSES_READ = -U lfuse:r:-:i -U hfuse:r:-:i -U efuse:r:-:i
...
fuses-read:
	$(DUDE) $(AVRDUDE_FLAGS) $(AVRDUDE_FUSES_READ)

# WARNING!!	A bad fuse setting can destroy your microcontroller.
# Pay attention and be safe that your fuses calculations are right. (for safety keep the command commented)
# Try this site http://www.engbedded.com/fusecalc/ a powerfull automated calculator for AVR chips.
fuses-write:
#	$(DUDE) $(AVRDUDE_FLAGS) -U lfuse:w:0x62:m -U hfuse:w:0xdf:m -U efuse:w:0xff:m
